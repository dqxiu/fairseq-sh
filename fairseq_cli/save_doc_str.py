...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.dataset.combine_valid_subsets:
        task.load_valid_dataset("valid", combine=True, epoch=1)
    else:
        for valid_sub_split in cfg.dataset.valid_subset.split(","):
            task.load_valid_dataset(valid_sub_split, combine=False, epoch=1)
...
def train(
    ...
    for i, samples in enumerate(progress):
        ...
        sentence = task.decode(samples[0]["net_input"]["src_tokens"][0])
        doc_str_list.append(sentence)

        if len(doc_str_list) >= cfg.task.train_data_num:
            should_stop = True
            break

    # save grad_cos and loss_diff
    res = {"doc_str_list": doc_str_list}
        ...
        with metrics.aggregate(new_root=True) as agg:
            for i, sample in enumerate(progress):
                if (
                    ...
                    break
                trainer.valid_step(sample, get_valid_grad=get_valid_grad)
